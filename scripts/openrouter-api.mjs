...
function generationBriefInstruction(body) {
  return JSON.stringify({
    ...
    rules: [
      ...
      "Спорность должна рождаться из честного конфликта: популярное обещание против реальной привычки, ожидание против ограничений, тренд против здравого смысла, громкий совет против проверяемой детали.",
      ...
      "Создавай shareable value: мини-диагностика ситуации, ошибка ожиданий, сравнение подходов, простая привычка, карта решения боли или проверяемая деталь из жизни.",
      ...
      "Обязательная проверка перед ответом: headline должен содержать конкретику из жизни аудитории, выведенную из продукта, хука и ниши. Если конкретики нет — перепиши.",
      ...
      "Не пиши сухие темы вроде 'что проверить' без боли. Преврати общий глагол в конкретную ситуацию, риск или маленький момент из жизни аудитории.",
...
function humanizeTextInstruction(body) {
  return JSON.stringify({
    ...
    rules: [
      ...
      "Сохраняй естественный порядок слов в русском заголовке: короткая фраза должна звучать как живой русский, а не как машинная подстановка слов.",
      ...
      "Используй простые речевые обороты, которые подходят конкретной нише и аудитории. Не копируй готовые сюжетные формулы между разными продуктами.",
      ...
      "Сохраняй безопасность: без обещаний решить любой случай, обойти правила или гарантировать результат."
    ],
    project: body.project,
    product: body.product,
