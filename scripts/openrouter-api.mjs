...
function generationBriefInstruction(body) {
  return JSON.stringify({
    ...
    output: {
      ...
      productInsightMap: {
        ...
        benefitZones: [
          {
            id: "короткий id зоны",
            pain: "жизненная боль или желание аудитории",
            habit: "смежная привычка или лайфхак, который помогает в той же зоне",
            safeFact: "безопасный факт о роли продукта или категории без медицинских/финансовых обещаний",
            visual: "понятный визуальный якорь для инфографики"
          }
        ],
    ...
    rules: [
      ...
      "Финальная самопроверка маркетолога: 1) заголовок короткий и конкретный, 2) пункты связаны с заголовком, 3) есть полезный смысл без покупки, 4) нет запрещенных обещаний, 5) пост можно сохранить или отправить другу.",
