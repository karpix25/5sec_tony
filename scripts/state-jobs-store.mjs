...
export async function loadJobs(query, appStateKey, options = {}) {
  ...
  const compactSql = `select id, project_id, product_id, character_id, status, stage, progress, title, topic, music, updated_at, ''::text as prompt, reference_title, output_type, final_video_url, final_video_has_audio, semantic_key, meaning_pattern_id, product_visual_mode, composition_mode, content_layer_id, format, input_urls, input_refs, diversity_slot, queue_name, queue_status, queue_priority, queue_attempts, queue_max_attempts, queue_scheduled_at, queue_locked_at, queue_lock_owner, queue_last_error, queue_idempotency_key, queue_provider_task_id, queue_metadata, extra - $${compactParams.length}::text[] as extra from studio_jobs where ${scope.where} order by updated_at desc nulls last, sort_order asc, id asc`;
  const fullSql = `select * from studio_jobs where ${scope.where} order by updated_at desc nulls last, sort_order asc, id asc`;
