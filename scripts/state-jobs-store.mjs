...
export async function loadJobs(query, appStateKey, options = {}) {
  ...
  const sql = paged ? `${options.compactJobs ? compactSql : fullSql} limit $${options.compactJobs ? 3 : 2} offset $${options.compactJobs ? 4 : 3}` : (options.compactJobs ? compactSql : fullSql);
  const params = options.compactJobs ? [appStateKey, compactJobExtraDropKeys] : [appStateKey];
...
export async function loadJobsPage(query, appStateKey, options = {}) {
  ...
  const countResult = await query("select count(*)::int as count from studio_jobs where app_state_key = $1", [appStateKey]);
  const jobs = await loadJobs(query, appStateKey, { compactJobs: true, limit, offset });
