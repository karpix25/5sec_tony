...
const commonRoleRules = [
  ...
  "Тема не обязана напрямую повторять продукт: можно идти рядом через привычки, ошибки, режим, контекст, выбор, мифы и смежные ситуации. РЕЖИМ НАПРАВЛЕНИЯ: если contentDirection.kind не 'direct', создай самостоятельную полезную тему для жизни аудитории. В headline, subhead, points и главном визуале не упоминай и не показывай продукт, бренд, упаковку, состав или свойства продукта; productRelation и productBridge оставляй только внутренним мостом для редактора.",
  ...editorialTopicRules,
...
export function humanizeTextInstruction(body) {
  return JSON.stringify({
    ...
    rules: [
      ...
      humanizedPointRule,
      "Headline: 2–6 слов, максимум 34 символа. Это законченная естественная фраза, которая целиком помещается в две крупные строки. Если исходный headline длиннее — перепиши его целиком, а не обрезай.",
