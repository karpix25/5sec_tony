...
export function fillProductDraft(form, draft) {
  ...
  setValue(form, "useCases", listText(normalized.useCases));
  setValue(form, "proofPoints", listText(normalized.proofPoints));
  setValue(form, "visualAnchors", listText(normalized.visualAnchors));
...
export function productPayloadFromDraft(base, draft = {}, references = []) {
  ...
  return {
    ...
    useCases: normalized.useCases || base.useCases || "",
    proofPoints: normalized.proofPoints || base.proofPoints || "",
    visualAnchors: normalized.visualAnchors || base.visualAnchors || "",
...
function normalizeProductDraft(draft = {}) {
  return {
    ...
    useCases: draft.useCases || draft.whenNeeded || draft.usageMoments || "",
    proofPoints: draft.proofPoints || draft.facts || draft.proof || "",
    visualAnchors: draft.visualAnchors || draft.visuals || draft.rituals || "",
