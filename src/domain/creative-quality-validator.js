...
function hasSpecificity(text) {
  return /япони|итал|рим|сингапур|дуба|оаэ|таиланд|франц|турци|гиалурон|коллаген|пептид|магни|витамин|чаев|капучино|эспрессо|влажн|крем|стакан|зел[её]н|карта|подписк|инвойс|счет|бронь|сервис|валют|статус|подтвержден|кофе|экран|свет|сон|турист|поездк|маршрут|жест|локальн|норм/.test(text);
}

function hasTension(text) {
  return /ошиб|не так|нелов|смути|выдает|лома|ожид|реальн|миф|вместо|после обеда|сух|риск|горит|отклон|последний час|не проходит|вслепую|странн/.test(text);
}

function hasNovelty(text) {
  if (vaguePhrases.some((phrase) => text.includes(phrase))) return false;
  return /факт|оказалось|неожидан|почему|в японии|в италии|в сингапуре|в таиланде|во франции|в оаэ|гиалуроновая кислота|хлорофилл|магний|витамин|коллаген|инвойс|подписка|бронь|страна карты|тип карты|статус заявки|патч-тест|пептид|поздний кофе|яркий экран|хаотичн.*сон|локальн.*норм|бытовой жест|местный контекст|первого дня маршрута/.test(text);
}
