...
const unsupportedClaimPatterns = [
  ...
  ["wellness_mechanism", /организм|клет|жкт|кишеч|вздут|пищевар|аппетит|самочувств|усваива|водн.{0,16}баланс|адаптац|накопительн|тяжест.{0,20}(?:еды|живот)|маскир.{0,16}запах|дезодор(?:ир|ант.{0,16}не\s+справл)|запах.{0,16}(?:тела|изнутри|изо рта)|свежест.{0,16}изнутри|внутренн.{0,16}(?:состояни|свежест|чистот)|энерги|тонус|иммун|кислород|митохондр|долгосрочн.{0,16}поддерж/iu],
  ...
  ["causal_certainty", /(?:главн|скрыт|прям).{0,16}причин|напрямую\s+влияет|зависит\s+от|дело\s+в|из\s+за|указывает\s+на.{0,20}дефицит|требу(?:ет|ют).{0,24}(?:белк|жирн|витамин|минерал)|связан.{0,24}(?:дискомфорт|проблем|наруш|бол)/iu],
  ["invented_diet_advice", /(?:меньше|мало).{0,20}движени.{0,28}(?:меньше\s+калори|сократ.{0,12}(?:порци|калори))|нужно\s+меньше\s+калори/iu],
  ["product_form", /таблетк|капсул|порош|сироп|спре[йя]|жидк.{0,8}концентрат/iu],
  ["quantified_comparison", /(?:два|две|три|четыре|пять|\d+)\s+раз(?:а)?\s+(?:меньше|больше|быстрее|дольше|эффективнее)/iu],
  ["skin_mechanism", /защитн.{0,16}барьер.{0,20}(?:ослаб|наруш|нужд)|(?:стянутост|морщин|тускл.{0,8}(?:тон|цвет)|сияни).{0,50}(?:означа|говор|указывает|из\s+за|дефицит|нехват|теря)|кож.{0,20}(?:не\s+старе|хочет\s+пить|становится\s+уязвим|теря.{0,12}(?:влаг|сияни))|гиалурон.{0,24}нужн.{0,16}влаг|сло[ия].{0,30}смешива.{0,20}скатыва/iu],
  ["cosmetic_effect", /не\s+забива.{0,16}пор|спаса.{0,24}кож|пита.{0,20}кож|освежа.{0,24}(?:цвет|тон).{0,12}лиц|идеальн.{0,20}(?:лож|нанос)|не\s+скатыва|удержива.{0,16}влаг/iu],
  ["external_causal_mechanism", /(?:ж[её]стк.{0,10}вод|отопительн.{0,12}сезон|сух.{0,10}воздух|экологи|питани|образ\s+жизни).{0,60}(?:оседа|вымыва|провоциру|вызыва|разруш|лишает|ухудша|дела.{0,14}(?:ж[её]стк|сух|тускл|ломк))/iu],
  ["device_misuse_harm", /(?:воздейств|массаж).{0,28}(?:кост|ше[ея]|позвоноч).{0,32}(?:боль|опас|травм)|(?:скорост|интенсивн).{0,32}(?:спазм|боль|травм)|(?:насадк|головк).{0,28}(?:чувствительн|ше[ея]).{0,32}(?:дискомфорт|боль|травм)/iu],
  ["effect", /обезвож|нормализ|стимулир|блокир|нейтрализ|избав|убира(?:ет|ют|ется)|крад[её]т|гарантир|навсегда|мгновенн|бодр|энергич|(?<!без)вред/iu]
];
