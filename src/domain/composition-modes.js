const compositionCatalog = {
  comparison: [
    ...
    {
      id: "expectation-detail-cards",
      label: "ожидание и деталь карточками",
      instruction: "Собери композицию карточками ожидание/проверяемая деталь или норма/ошибка. Карточки могут идти каскадом, но не как один длинный список."
    },
    {
      id: "comparison-table",
  ...
  ranking_leaderboard: [
    {
      id: "leaderboard-bars",
      label: "рейтинг с вертикальными колонками",
      instruction: "Собери композицию как leaderboard: повторяемые ранжированные колонки или карточки с номером, коротким value label, визуальным слотом и подписью. Не превращай это в обычный список советов."
    },
    {
      id: "ranked-card-grid",
      label: "сетка ранжированных карточек",
      instruction: "Собери композицию как плотную сетку rank-card элементов: каждый элемент имеет место, номер, короткий критерий и компактный визуальный маркер."
    },
    {
      id: "top-chart-poster",
      label: "постер топ-чарта",
      instruction: "Собери композицию как топ-чарт с большим заголовком, служебной строкой/легендой и повторяемыми ранговыми блоками. Сохрани ощущение таблицы достижений, но адаптируй смысл под продукт."
    }
  ],
  default: [
    {
