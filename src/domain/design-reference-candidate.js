...
export function buildDesignReferenceCandidatePrompt(project = {}, payload = {}) {
  ...
  return [
    "GPT Image 2: создай вертикальный 9:16 дизайн-шаблон для будущих инфографик коротких видео.",
    "Это только шаблон стиля, не готовая реклама и не финальный контент.",
    "Используй композицию по центру кадра. Сохраняй щедрые чистые отступы по всем краям.",
    "Оставь верхнее и нижнее поля полностью чистыми и просторными, чтобы будущие интерфейсные оверлеи соцсетей не перекрывали контент.",
    "Используй только короткий русский placeholder-текст; не используй готовый рекламный текст.",
    "Не показывай упаковку продукта, аватара, человека, лицо, руки, логотип бренда, этикетку продукта, водяной знак, QR-код, CTA-кнопку или интерфейс соцсетей.",
    "Не копируй реальный текст конкурента, claim, продукт, логотип или обещание.",
    "Видимый текст может быть только короткими нейтральными русскими заглушками для иерархии: Заголовок, Пункт 1, Пункт 2.",
    "Держи placeholder-текст крупным, чистым и читаемым; без псевдолатинского текста, без английских служебных подписей, без мелких абзацев.",
    "Создай переиспользуемый инфографический макет: зона хука, supporting-блоки, зона визуального объекта, ритм, отступы, палитра и типографика.",
    ...
    project.companyInfo ? `Контекст компании: ${project.companyInfo}.` : "",
    project.companyAudience ? `Аудитория: ${project.companyAudience}.` : "",
    `Инструкция по стилю: ${prompt}.`,
    payload.layoutType ? `Тип макета: ${payload.layoutType}.` : "",
    payload.palette ? `Палитра: ${payload.palette}.` : "",
    `Типографика: ${fontStyle}.`,
