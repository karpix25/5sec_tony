...
const weakHeadlineShellPattern = /^(?:почему\s+|скрип\s+|не\s+просто\s+|ошибка\s+(?:в|для|при)(?:\s|$)|(?:содержит|содержат|в\s+составе|обогащ[её]н(?:а|о|ы)?|формула\s+с|с\s+(?:ароматом|вкусом))(?:\s|$)|(?:мягкий|приятный|натуральный|свежий|л[её]гкий)\s+(?:вкус|аромат(?:изатор)?|состав|текстура)(?:\s|$)|(?:ошибки|советы|правила|основы|особенности|признаки|гайд|чек-?лист)\s+(?:при|для|по|о|в)(?:\s|$)|(?:вы|ты)\s+(?:это|их|его|е[её])(?:\s|$)|вот\s+что|разбираемся|миф(?:ы)?\s+(?:о|про)|что\s+важно\s+знать|важн(?:ый|ые)\s+факт|полезн(?:ый|ые)\s+(?:совет|факт)|эту\s+деталь\s+(?:легко\s+)?(?:упустить|не\s+заметить)|это\s+не\s+(?:норма|работает)$|(?:ваш|твой)\s+.+?\s+(?:—\s*)?это\s+(?:просто\s+)?маркетинг$)/i;
