...
test("generation job keeps prompt contract, trace and active product refs together", () => {
  const job = createGenerationJob({
    ...
    generationBrief: {
      ...
      contentScript: { headline: "Проверь эту привычку заранее", subhead: "одна строка", points: ["Вода утром"] },
  ...
  assert.equal(job.promptContract.textContract.headline, "Проверь эту привычку заранее");
  assert.equal(job.aiTrace.selectedHook, "Проверь эту привычку заранее");
