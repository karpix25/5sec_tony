...
test("visible text contract rejects generic and broken editorial shells", () => {
  for (const headline of ["Почему внешние средства", "Вы их буквально ломаете", "Исправляем эргономику питания", "Ошибки при сушке феном", "Ошибка в вечерней рутине", "Не просто увлажнение", "Мягкий вкус мяты", "Натуральный ароматизатор мяты", "Содержит гидроксиапатит и ксилит", "В составе масло ши", "Формула с пантенолом", "С ароматом свежей мяты", "Скрип зубов — не признак чистоты", "Вот что важно знать", "Разбираемся, чего не хватает", "Миф о том, что натуральный состав"]) {
